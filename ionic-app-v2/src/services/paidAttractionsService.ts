...
// Données mockées retirées - utiliser uniquement les données du backend
...
export const attractionsService = {
  ...
  async getPaidAttractions(useCache: boolean = true): Promise<PaidAttraction[]> {
    try {
      ...
      // Appel API réel - récupérer les attractions payantes depuis le backend
      const response = await fetch(`${import.meta.env.VITE_API_URL}/attractions?paid=true`);
      ...
      console.error('Error fetching paid attractions:', error);
  ...
  async getUserPurchases(userId: string): Promise<string[]> {
    try {
      ...
      // Appel API réel - récupérer les achats de l'utilisateur
      const response = await fetch(`${import.meta.env.VITE_API_URL}/users/${userId}/purchases`);
      
      if (!response.ok) {
        if (response.status === 404) {
          return []; // Utilisateur sans achats
        }
        throw new Error(`HTTP error! status: ${response.status}`);
      }

...
exports.trackAction = exports.getDashboardAnalytics = exports.compareWithPeers = exports.getUserTrends = void 0;
...
const compareWithPeers = async (req, res) => {
    try {
        ...
        const userStats = await UserStats_1.default.findOne({ userId });
        if (!userStats) {
        ...
        const allStats = await UserStats_1.default.find().select('attractionsVisited audioGuidesListened toursCompleted totalListeningTime favoriteCount reviewCount shareCount');
        ...
        const averages = allStats.reduce((acc, stats) => ({
        ...
        Object.keys(averages).forEach((key) => {
            averages[key] = Math.round(averages[key] / totalUsers);
        });
        // Calculer le rang de l'utilisateur (basé sur attractionsVisited)
        const sortedStats = allStats.sort((a, b) => b.attractionsVisited - a.attractionsVisited);
...
exports.compareWithPeers = compareWithPeers;
/**
...
const getDashboardAnalytics = async (req, res) => {
    try {
        // Stats globales
        const [totalUsers, totalAttractions, totalActivities, recentActivities] = await Promise.all([
            UserStats_1.default.countDocuments(),
            Attraction_1.Attraction.countDocuments({ status: 'active' }),
        ...
        const topAttractions = await Attraction_1.Attraction.find({ status: 'active' })
            .sort({ visitCount: -1 })
            .limit(10)
            .select('name visitCount rating');
        // Top utilisateurs (par attractions visitées)
        const topUsers = await UserStats_1.default.find()
        ...
        const activityByType = await ActivityLog_1.default.aggregate([
...
const trackAction = async (req, res) => {
    try {
        const { userId, action, attractionId, audioGuideId, tourId, metadata } = req.body;
        // Validation
        if (!userId || !action) {
            return res.status(400).json({
                success: false,
        ...
        let userActivity = await UserActivity_1.default.findOne({
            userId,
            date: today,
        });
        if (!userActivity) {
            userActivity = new UserActivity_1.default({
                userId,
                date: today,
            });
        }
